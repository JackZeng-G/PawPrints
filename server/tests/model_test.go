...
func TestModel_DiaryEntry(t *testing.T) {
	diary := model.DiaryEntry{Title: "今天很开心", Content: "小橘今天学会了跳高", EntryDate: time.Now(), Mood: "happy"}
...
func TestModel_HealthRecord(t *testing.T) {
	hr := model.HealthRecord{PetID: 1, Type: "vaccine", Title: "狂犬疫苗", RecordDate: time.Now()}
	if hr.Type != "vaccine" {
...
func TestModel_Reminder(t *testing.T) {
	rem := model.Reminder{Title: "疫苗接种提醒", RemindAt: time.Now().AddDate(0, 0, 7), Type: "vaccine_due", PetID: 1, Completed: false}
	if rem.Title != "疫苗接种提醒" {
